...
function simplify(ex::Expr)
    ...
    if all(isnumber, ex.args[2:end]) && length(ex.args) > 1
        return (@static (VERSION < v"0.7.0-DEV.5149") ? eval : Core.eval)(Main, ex)#this is a hack to eval in Main, but seems to work well -- maybe in the future add the ability to specify the module for this when doing @NonlinearEquations.equations. Calculus.jl's desire to eval in @__MODULE__ leads to crashes
    end
